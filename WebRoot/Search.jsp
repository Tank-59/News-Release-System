...
<div id="wrapper">
	...
	<div id="main">
		...
		<c:when test="${page.hasPrePage}">
			<a href="/News/News_Search?method=list&currentPage=1&select=${tt }&textfield=${txt }">首页</a> | 
			<a href="/News/News_Search?method=list&currentPage=${page.currentPage -1 }&select=${tt }&textfield=${txt }">上一页</a>
		</c:when>
		<c:otherwise>
			首页 | 上一页
		</c:otherwise>
	...
	<c:choose>
		<c:when test="${page.hasNextPage}">
			<a href="/News/News_Search?method=list&currentPage=${page.currentPage + 1 }&select=${tt }&textfield=${txt }">下一页</a> | 
			<a href="/News/News_Search?method=list&currentPage=${page.totalPage }&select=${tt }&textfield=${txt }">尾页</a>
		</c:when>
		<c:otherwise>
			下一页 | 尾页
		</c:otherwise>
